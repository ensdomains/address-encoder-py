...
from address_encoder.async_ import get_coder_by_coin_name_async, get_coder_by_coin_type_async
...
def test_coin_name() -> None:
    coder = asyncio.run(get_coder_by_coin_name_async("btc"))
    assert coder.coin_type == 0
    assert coder.name == "btc"


def test_coin_type() -> None:
    coder = asyncio.run(get_coder_by_coin_type_async(0))
    assert coder.coin_type == 0
    assert coder.name == "btc"


def test_evm_coin_name() -> None:
    coder = asyncio.run(get_coder_by_coin_name_async("op"))
...
def test_evm_coin_type() -> None:
    coder = asyncio.run(get_coder_by_coin_type_async(2147483658))
    assert coder.coin_type == 2147483658
    assert coder.name == "op"
    assert coder.evm_chain_id == 10
...
def test_unknown_evm_coin_type() -> None:
    coder = asyncio.run(get_coder_by_coin_type_async(2147483659))
...
def test_get_coder_by_coin_name_async_non_evm(coin_name: str) -> None:
    coder = asyncio.run(get_coder_by_coin_name_async(coin_name))
...
def test_get_coder_by_coin_name_async_evm(coin_name: str) -> None:
    coder = asyncio.run(get_coder_by_coin_name_async(coin_name))
    assert coder.name == coin_name
...
def test_get_coder_by_coin_type_async_non_evm(coin_type: int) -> None:
    coder = asyncio.run(get_coder_by_coin_type_async(coin_type))
...
def test_get_coder_by_coin_type_async_evm(coin_type: int) -> None:
    coder = asyncio.run(get_coder_by_coin_type_async(coin_type))
